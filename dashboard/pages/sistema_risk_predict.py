...
def show():
    ...
    uploaded_file = st.file_uploader(
        "Upload do CSV",
        type="csv",
        help="O arquivo deve estar no formato .csv, separado por ponto e vírgula (;). Certifique-se de que ele possui as colunas: ID_CLIENTE, SAFRA_REF, DATA_EMISSAO_DOCUMENTO, DATA_VENCIMENTO, VALOR A PAGAR e TAXA.",
    )
    ...
    if uploaded_file is not None:
        ...
        top_10 = submissao_case.sort_values(
            "PROBABILIDADE_INADIMPLENCIA", ascending=False
        ).head(10)
        ...
        fig_hist.update_layout(
            xaxis_title="Probabilidade", yaxis_title="Frequência", plot_bgcolor="white"
        )
        ...
        fig_pizza.update_layout(
            title="Proporção de clientes com risco ≥ 50%", plot_bgcolor="white"
        )
